...
- Phase 1 与来源探测的可靠离线测试已通过：34/34。
- 网络探测候选池已由 28 项扩为 152 项，全部仍是未激活候选；9 个 ECS 连接/TLS 异常项已形成独立、严格匹配主目录的 GitHub-hosted Ubuntu 对照目录。
...
- GitHub Actions 外部节点尚未产出第一次 9-source 对照结果；工作流启用后以其 artifact 为证据，不以工作流文件存在视为已经完成对照。
...
- 公网 DNS 已恢复，但部分来源仍存在地址族/路由、TLS 超时、连接重置、HTTP 拒绝或缺少参数。一次成功不证明长期稳定；在许可、速率和外部节点对照完成前不把候选源标为已验证或直接启用。
